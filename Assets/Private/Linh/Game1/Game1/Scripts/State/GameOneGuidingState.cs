...
namespace Monkey.Game.GameOneDemo
{
    ...
    {
        private GameOneGuidingDependency dependency;
        private CancellationTokenSource cts;
        private ButtonAnswerController buttonCorrect;
        private bool isGuiding = false;
        public override void SetUp(object data)
        {
            dependency = (GameOneGuidingDependency)data;
        }
        ...
        public async void StartGuidingClick(bool isDelay)
        {
            ...
            try
            {
                foreach (var item in dependency.ButtonAnswerControllers)
                {
                    if (item.IsCorrect)
                    {
                        buttonCorrect = item;
                ...
                if (isDelay) await UniTask.Delay(dependency.GameOneGuidingConfig.timeDelayStart, cancellationToken: cts.Token);
                ...
                while (isGuiding)
                {
                    ...
                    await UniTask.Delay(dependency.GameOneGuidingConfig.timeDelay, cancellationToken: cts.Token);

                    dependency.UiGuiding.DOFade(1f, 0.35f).SetEase(Ease.Linear).onComplete += () => { tscFadeDone = true; };
                    soundData = new SoundChannel(SoundChannel.PLAY_SOUND_NEW_OBJECT, dependency.GameOneGuidingConfig.sfxAppear);
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        soundData = new SoundChannel(SoundChannel.PLAY_SOUND_NEW_OBJECT, dependency.GameOneGuidingConfig.sfxClick);
                        ObserverManager.TriggerEvent<SoundChannel>(soundData);
                        SetColorImage(dependency.HandPoint, 0f);
                        SetColorImage(dependency.HandHold, 1f);
                        await UniTask.Delay(dependency.GameOneGuidingConfig.timeDelay * 2, cancellationToken: cts.Token); ;
                        soundData = new SoundChannel(SoundChannel.PLAY_SOUND_NEW_OBJECT, dependency.GameOneGuidingConfig.sfxUnClick);
                        ObserverManager.TriggerEvent<SoundChannel>(soundData);
                    ...
                    await UniTask.WaitUntil(() => tscFadeDone, cancellationToken: cts.Token);
                    await UniTask.Delay(dependency.GameOneGuidingConfig.timeDelayStart, cancellationToken: cts.Token);
    ...
    public class GameOneGuidingDependency
    {
        public GameOneGuidingConfig GameOneGuidingConfig { get; set; }
