...
{
    public class MTCCAManager : GameManager, EventListener<StateChanel>
    {
        protected override void Start()
        {
            ...
            MTCCAInitStateData initStateData = adapter.GetData<MTCCAInitStateData>(StaticValue.CurrentTurn);
        ...
        public void OnMMEvent(StateChanel eventType)
        {
            switch (eventType.StatusOfState)
            {
                case StateName.Status.InitFinish:
                    MTCCAIntroStateData introStateData = adapter.GetData<MTCCAIntroStateData>(-5858);
